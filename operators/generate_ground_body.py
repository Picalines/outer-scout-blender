...
class OW_RECORDER_OT_generate_ground_body(Operator, GroundBodySelectionHelper):
    ...
    def execute(self, context):
        ...
        self._log(
            "INFO",
            f"importing {len(imported_ow_objects)} .obj files of streamed assets",
        )
        ...
        for sector_index, sector_info in enumerate(ground_body_meshes_info["sectors"]):
            self._log(
                "INFO",
                f"* sector '{sector_info['path']}' [{sector_index + 1}/{sectors_count}]",
            )
            ...
            for plain_mesh_info in sector_info["plain_meshes"]:
                ...
                if extracted_child is None:
                    self._log(
                        "WARNING",
                        f'missing plain mesh child at {plain_mesh_info["path"]}',
                    )
                    continue
        ...
        self._log(
            "INFO",
            f"clearing scene collection ({len(context.scene.collection.objects)} objects)",
        )
