...
class OW_RECORDER_OT_synchronize(Operator):
    ...
    def _sync_blender_to_ow(self, context: Context):
        ...
        new_transform_local_to_ground_body = (
            ground_body.matrix_world.inverted() @ blender_item_matrix
        )
        new_transform_local_to_ground_body = TransformModel.from_matrix(
            new_transform_local_to_ground_body
        )
        new_transform_local_to_ground_body = (
            new_transform_local_to_ground_body.blender_to_unity()
        )

        success = api_cilent.set_transform_local_to_ground_body(
            self.ow_item, new_transform_local_to_ground_body
        )
        if success is False:
